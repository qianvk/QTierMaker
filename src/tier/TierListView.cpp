...
qreal missionMacHoverEase(qreal value) {
    value = qBound<qreal>(0.0, value, 1.0);
    const qreal softStart = missionSmoothStep(value);
    const qreal criticallyDamped = 1.0 - std::pow(1.0 - value, 3.35);
...
void TierListView::updateInteractionMouseTracking() {
    const bool enabled = m_missionControlActive || m_missionTransitionProgress > 0.001;
...
void TierListView::animateMissionHover(qreal targetProgress) {
    ...
    // Keep the QVariantAnimation linear and apply the macOS-like easing inside
    // the layout function, so size, position, and collision projection share one
    // consistent timing curve.
...
QString TierListView::blankAreaToolTipAt(const QPoint& viewportPoint) const {
    ...
    if (delegate->labelRect(rowRect).contains(viewportPoint)) {
        return tr("Double-click to display this tier. Right-click for tier actions.");
    }
