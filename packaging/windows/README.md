...
The install step runs Qt's deployment API before CPack creates a DPI-aware machine-wide
installer. It
...
before packaging and signs the final packages. A `vX.Y.Z` tag publishes the x64 installer and
executable update package; their SHA-256 hashes are recorded in `updates.json`.
